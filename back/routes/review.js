...
async function activeFeedbackQuery(objectId) {
  ...
  const allFeedbackQuery = new Parse.Query('Preference').equalTo(
    'forExperience',
    true,
  );
  const allFeedback = await allFeedbackQuery.find();
  const allFeedbackIDs = allFeedback.map(
...
router.post('/active/info', async (req, res) => {
  ...
  const activeFeedbackInfo = activeFeedbackIDs.map((idActive) =>
    allFeedback
      .find((feedback) => feedback.toJSON().objectId === idActive)
      .toJSON(),
  );
...
router.post('/active', async (req, res) => {
  ...
  const activeReviews = reviews.filter((review) =>
    activeFeedback.includes(review.feedbackId),
  );
